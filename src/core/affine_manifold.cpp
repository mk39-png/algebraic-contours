...
// ***********************
//  JSON MAKING (generated by chatgpt)
// VertexManifoldChart
// ***********************
// Below was generated
// Serialize vector<T> as JSON array
template<typename T>
std::string to_json_array(const std::vector<T>& vec) {
...
// Serialize Eigen::MatrixXf as 2D JSON array
std::string to_json_matrix(const MatrixXr& mat) {
    std::ostringstream oss;
    oss << "[";
...
// Convert single VertexManifoldChart to JSON
std::string to_json(const VertexManifoldChart& chart) {
    std::ostringstream oss;
    oss << "{";
    oss << "\"vertex_index\": " << chart.vertex_index << ", ";
    oss << "\"vertex_one_ring\": " << to_json_array(chart.vertex_one_ring) << ", ";
    oss << "\"face_one_ring\": " << to_json_array(chart.face_one_ring) << ", ";
    oss << "\"one_ring_uv_positions\": " << to_json_matrix(chart.one_ring_uv_positions) << ", ";
    oss << "\"is_boundary\": " << (chart.is_boundary ? "true" : "false") << ", ";
    oss << "\"is_cone\": " << (chart.is_cone ? "true" : "false") << ", ";
    oss << "\"is_cone_adjacent\": " << (chart.is_cone_adjacent ? "true" : "false");
    oss << "}";
    return oss.str();
}

// Convert vector of VertexManifoldChart to JSON
std::string to_json(const std::vector<VertexManifoldChart>& charts) {
    std::ostringstream oss;
    oss << "[";
    for (size_t i = 0; i < charts.size(); ++i) {
        oss << to_json(charts[i]);
        if (i + 1 < charts.size())
            oss << ", ";
    }
    oss << "]";
...
// ************
// EdgeManifoldChart generated by chatgpt 
// ************
// Serialize PlanarPoint as JSON array
std::string to_json_point(const PlanarPoint& p) {
    std::ostringstream oss;
    oss << "[" << p[0] << ", " << p[1] << "]";
    return oss.str();
}

// Convert single EdgeManifoldChart to JSON
std::string to_json(const EdgeManifoldChart& chart) {
    std::ostringstream oss;
    oss << "{";
    oss << "\"top_face_index\": " << chart.top_face_index << ", ";
    oss << "\"bottom_face_index\": " << chart.bottom_face_index << ", ";
    oss << "\"left_vertex_index\": " << chart.left_vertex_index << ", ";
    oss << "\"right_vertex_index\": " << chart.right_vertex_index << ", ";
    oss << "\"top_vertex_index\": " << chart.top_vertex_index << ", ";
    oss << "\"bottom_vertex_index\": " << chart.bottom_vertex_index << ", ";
    oss << "\"left_vertex_uv_position\": " << to_json_point(chart.left_vertex_uv_position) << ", ";
    oss << "\"right_vertex_uv_position\": " << to_json_point(chart.right_vertex_uv_position) << ", ";
    oss << "\"top_vertex_uv_position\": " << to_json_point(chart.top_vertex_uv_position) << ", ";
    oss << "\"bottom_vertex_uv_position\": " << to_json_point(chart.bottom_vertex_uv_position) << ", ";
    oss << "\"is_boundary\": " << (chart.is_boundary ? "true" : "false");
    oss << "}";
    return oss.str();
}

// Convert vector of EdgeManifoldChart to JSON
std::string to_json(const std::vector<EdgeManifoldChart>& charts) {
    std::ostringstream oss;
    oss << "[";
    for (size_t i = 0; i < charts.size(); ++i) {
        oss << to_json(charts[i]);
...
// ************************************
// FaceManifoldChart by chatgpt
// ************************************
// Convert array<PlanarPoint, 3> to JSON array of arrays
std::string to_json_array(const std::array<PlanarPoint, 3>& arr) {
    std::ostringstream oss;
    oss << "[";
    for (size_t i = 0; i < arr.size(); ++i) {
        oss << to_json_point(arr[i]);
        if (i + 1 < arr.size()) oss << ", ";
    }
    oss << "]";
    return oss.str();
...
// Convert array<bool, 3> to JSON array
std::string to_json_array(const std::array<bool, 3>& arr) {
    std::ostringstream oss;
    oss << "[";
    for (size_t i = 0; i < arr.size(); ++i) {
...
// Convert a single FaceManifoldChart to JSON
std::string to_json(const FaceManifoldChart& chart) {
    std::ostringstream oss;
    oss << "{";
    oss << "\"face_index\": " << chart.face_index << ", ";
    oss << "\"face_uv_positions\": " << to_json_array(chart.face_uv_positions) << ", ";
    oss << "\"is_boundary\": " << (chart.is_boundary ? "true" : "false") << ", ";
    oss << "\"is_cone_adjacent\": " << (chart.is_cone_adjacent ? "true" : "false") << ", ";
    oss << "\"is_cone_corner\": " << to_json_array(chart.is_cone_corner);
    oss << "}";
    return oss.str();
}

// Convert vector of FaceManifoldChart to JSON
std::string to_json(const std::vector<FaceManifoldChart>& charts) {
    std::ostringstream oss;
    oss << "[";
    for (size_t i = 0; i < charts.size(); ++i) {
        oss << to_json(charts[i]);
        if (i + 1 < charts.size()) oss << ", ";
    }
    oss << "]";
    return oss.str();
}

void serialize_vertex_charts(std::string filename, std::vector<VertexManifoldChart> charts) {
    spdlog::info("Writing vertex charts data to {}", filename);
    std::ofstream output_file(filename, std::ios::out | std::ios::trunc);

    output_file << to_json(charts);
    output_file.close();
}



void serialize_edge_charts(std::string filename, std::vector<EdgeManifoldChart> charts) {
    spdlog::info("Writing edge charts data to {}", filename);
    std::ofstream output_file(filename, std::ios::out | std::ios::trunc);
...
void serialize_face_charts(std::string filename, std::vector<FaceManifoldChart> charts) {
    spdlog::info("Writing face charts data to {}", filename);
    std::ofstream output_file(filename, std::ios::out | std::ios::trunc);
  ...
  , m_F_uv(F_uv)
{
  ...
  // NOTE: m_halfedge has been serialized internally.
  ...
  // NOTE: this changes m_vertex_charts

  // Mark vertices, edges, and faces adjacent to cones
  mark_cones();
  // NOTE: this changes m_vertex_charts and m_face_charts
  ...
  // XXX: serializing the member variables 
  // std::string filepath = "spot_control/affine_manifold/";
  // serialize_eigen_matrix_i(filepath+"F.csv", F);
  // serialize_eigen_matrix_d(filepath+"global_uv.csv", global_uv);
  // serialize_eigen_matrix_i(filepath+"F_uv.csv", F_uv);
  // serialize_vector_vector(filepath+"corner_to_he.csv", m_corner_to_he); // from halfedge
  // serialize_vector_pair_index(filepath+"he_to_corner.csv", m_he_to_corner); // from halfedge
  // serialize_vector_int(filepath+"he_to_edge.csv", he_to_edge);
  // serialize_vector_vector(filepath+"corner_to_edge.csv",m_corner_to_edge);
  // serialize_vector_vector(filepath+"l.csv", m_l);

  // serialize_vertex_charts(filepath+"vertex_charts_aligned.json", m_vertex_charts); // TODO: test before align_local_charts and before mark_cones()
  // serialize_edge_charts(filepath+"edge_charts_aligned.json", m_edge_charts); // test right after build_edge_charts_from_lengths
  // serialize_face_charts(filepath+"face_charts_aligned.json", m_face_charts);// TODO: test before mark_cones and right after build_face_charts
  // std::cout << m_face_charts.at(0).is_cone_corner[0] <<  m_face_charts.at(0).is_cone_corner[1] <<  m_face_charts.at(0).is_cone_corner[2] << std::endl;
}
